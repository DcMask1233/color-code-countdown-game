...
interface SecureUnifiedGameEngineProps {
  gameType: "Parity" | "Sapre" | "Bcone" | "Emerd";
  gameMode: "Wingo1min" | "Wingo3min" | "Wingo5min";
}

export const SecureUnifiedGameEngine = ({ gameType, gameMode }: SecureUnifiedGameEngineProps) => {
  const { currentPeriod, timeLeft, isLoading, error } = useGamePeriods(gameType, gameMode);
  const { userBets, placeBet, isLoading: isBetLoading } = useSecureGameEngine(gameType, gameMode);
